...
func (api *MemoryAPI) SaveUser(ctx context.Context, user auth.User) error {
	return api.repo.Save(ctx, user) //nolint:wrapcheck // thin wrapper, repo error is sufficient
}

func (api *MemoryAPI) FindUserByID(ctx context.Context, id auth.UserID) (auth.User, error) {
	return api.repo.FindByID(ctx, id) //nolint:wrapcheck // thin wrapper, repo error is sufficient
}
