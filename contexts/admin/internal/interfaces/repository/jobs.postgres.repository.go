...
func (repo *PostgresJobsRepository) PendingJobs(ctx context.Context, queue jobs.QueueName) ([]jobs.Job, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: could not get pending jobs: %v", postgres.ErrQueryFailed, err)
	}
...
func (repo *PostgresJobsRepository) QueueKPIs(ctx context.Context, queue jobs.QueueName) (jobs.QueueKPIs, error) { //nolint:funlen,lll
	...
	group.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("%w: could not query pending jobs: %v", postgres.ErrQueryFailed, err)
		}
	...
	group.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("%w: could not query failed jobs: %v", postgres.ErrQueryFailed, err)
		}
	...
	group.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("%w: could not query processed jobs: %v", postgres.ErrQueryFailed, err)
		}
	...
	group.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("%w: cound not query pending job_types: %v", postgres.ErrQueryFailed, err)
		}
	...
	group.Go(func() error {
		...
		if err != nil {
			return fmt.Errorf("%w: could not query total queue worker size: %v", postgres.ErrQueryFailed, err)
		}
...
func (repo *PostgresJobsRepository) DeleteByID(ctx context.Context, jobID string) error {
	...
	if err != nil {
		return fmt.Errorf("%w: could not delete: %v", jobs.ErrJobLockedAlready, err)
	}
...
func (repo *PostgresJobsRepository) RunJobAt(ctx context.Context, jobID string, runAt time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("%w: could not reschedule: %v", jobs.ErrJobLockedAlready, err)
	}
...
func (repo *PostgresJobsRepository) WorkerPools(ctx context.Context) ([]jobs.WorkerPool, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %v", postgres.ErrQueryFailed, err)
	}
...
func (repo *PostgresJobsRepository) Schedules(ctx context.Context) ([]jobs.Schedule, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: could not get schdules: %v", postgres.ErrQueryFailed, err)
	}
...
func (repo *PostgresJobsRepository) FinishedJobs(ctx context.Context, filter jobs.Filter) ([]jobs.Job, error) {
	...
	if filter.Queue != "" && filter.JobType != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: could not get finished jobs by queue and job type: %v", postgres.ErrQueryFailed, err)
		}
	...
	if filter.Queue != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("%w: could not get finished jobs by queue: %v", postgres.ErrQueryFailed, err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("%w: could not get finished jobs: %v", postgres.ErrQueryFailed, err)
	}
...
func (repo *PostgresJobsRepository) FinishedJobsTotal(ctx context.Context, filter jobs.Filter) (int64, error) {
	...
	if filter.Queue != "" && filter.JobType != "" {
		...
		if err != nil {
			return 0, fmt.Errorf("%w: %v", postgres.ErrQueryFailed, err)
		}
